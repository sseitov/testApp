...
static int open_input_file(const char *filename)
{
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (stream->codecpar->codec_id == 0) { // codec none
            continue;
        }
        AVCodec *dec = avcodec_find_decoder(stream->codecpar->codec_id);
        AVCodecContext *codec_ctx;
        if (!dec) {
            av_log(NULL, AV_LOG_ERROR, "Failed to find decoder for stream #%s (%u)\n", avcodec_get_name(stream->codecpar->codec_id), stream->codecpar->codec_id);
...
static int open_output_file(const char *filename, NSDictionary* info)
{
    AVStream *out_stream;
    AVStream *in_stream;
    ...
    for (i = 0; i < ifmt_ctx->nb_streams; i++) {
        ...
        if (dec_ctx->codec_type == AVMEDIA_TYPE_VIDEO || dec_ctx->codec_type == AVMEDIA_TYPE_AUDIO) {
            enum AVCodecID encoderID = dec_ctx->codec_id;
            if (encoderID == AV_CODEC_ID_HEVC) {
                encoderID = AV_CODEC_ID_H264;
            }
            encoder = avcodec_find_encoder(encoderID);
            if (!encoder) {
...
+ (BOOL)transcode:(NSString*)inFile outFile:(NSString*)outFile meta:(NSDictionary*)meta {
    ...
    if ((ret = open_input_file(inFile.UTF8String)) < 0)
        goto end;
    if ((ret = open_output_file(outFile.UTF8String, meta)) < 0)
        goto end;
    ...
    while (1) {
        ...
        if (packet.stream_index != videoIndex && packet.stream_index != audioIndex) {
            ...
            av_log(NULL, AV_LOG_ERROR, "Skip unused packet\n");
            continue;
        }
        totalBytesRead += packet.size;
        av_log(NULL, AV_LOG_ERROR, "totalBytesRead %d\n", totalBytesRead);
        ...
        if (filter_ctx[stream_index].filter_graph) {
            ...
            if (ret < 0)
                ...
                av_log(NULL, AV_LOG_ERROR, "write without transcoding %d\n", ret);
